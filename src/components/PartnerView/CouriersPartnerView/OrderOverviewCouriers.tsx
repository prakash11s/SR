...
const PartnerOrderOverview = (): JSX.Element => {
      ...
      dispatch(
        getPartnerAvailableOrders(id, (status, res) => {
          if (status) {
            ...
            getMetaNames(res.additional_data);
          } else {
            setError(res);
          }
        })
  ...
  const getMetaNames = (additionalData) => {
    additionalData.forEach((data: any) => {
      if (data.key === "transportation_vehicle_id") {
        dispatch(
          getVehicleName(data.json_value["id"], (status, res) => {
            if (status) {
              setVehicleTypeName(res.name);
            } else {
              setVehicleTypeName(res);
            }
          })
        );
      } else if (data.key === "cargo_type_id") {
        dispatch(
          getCargoName(data.json_value["id"], (status, res) => {
            if (status) {
              setCargoTypeName(res.name);
            } else {
              setCargoTypeName(res);
            }
          })
        );
      }
                        ...
                        <div key={data.key}>
                          {data.key === "dimensions" && (
                            <h5>
                              ...
                              {data.json_value["width"]} &nbsp;
                              <IntlMessages id="Height" />:{" "}
                              {data.json_value["height"]} &nbsp;
                              <IntlMessages id="Length" />:{" "}
                              {data.json_value["length"]} &nbsp;
                              <IntlMessages id="Weight" />:{" "}
                              {data.json_value["weight"]} &nbsp;
                            </h5>
                          )}
                          {data.key === "route_information" && (
                            <>
                              <h5>
                                ...
                                {data.json_value["origin"]["full"]} &nbsp;
                              </h5>
                              <h5>
                                <b>
                                  <IntlMessages id="toLocation" />:{" "}
                                </b>{" "}
                                {data.json_value["destination"]["full"]} &nbsp;
                              </h5>
                              <h5>
                                <b>
                                  <IntlMessages id="receiver.distance" />:{" "}
                                </b>{" "}
                                {data.json_value["distance"]
                                  ? distanceInKm(data.json_value["distance"])
                                  : ""}{" "}
                                &nbsp;
                              </h5>
                              <h5>
                                ...
                                {data.json_value["duration"]
                                  ? secondsToHms(data.json_value["duration"])
                                  : ""}{" "}
                                &nbsp;
                              </h5>
                            </>
                          )}
                          {data.key === "cargo_type_id" && (
                            <h5>
                              <b>
                          ...
                          {data.key === "transportation_vehicle_id" && (
                            <h5>
                              <b>
      ...
      >
        ...
        >
          <IntlMessages
            ...
            values={{
              termsandconditions: (
                <a href="https://serviceright-koeriers.nl/algemene-voorwaarden-bedrijven/">
