...
import {
  ...
  IServiceEdit,
} from "./Interface/Step4Interface";
import IntlMessages from "../../../util/IntlMessages";
import moment from "moment";
import queryString from "query-string";


const VehiclesStep4 = (props: IVehiclesStep4Props) => {
  /**
   *  dispatch
   */
  ...
  const prepareData = (fieldName: string) => {
    ...
    const license = "license-plate";

    let payload: any = {
      vehicle_id: orderState.vehicle.vehicleData.vehicle_id,
      ...
      construction_year: orderState.vehicle.construction_year,
      delivery_option_id: orderDetails.dataAssignment.deliveryOption ? orderDetails.dataAssignment.deliveryOption : null,
    };
    ...
    // only add the plate id in case we have it set
    if (
      orderState.vehicle.vehicleData.plate_id &&
      orderState.vehicle.vehicleData.plate_id.length > 2
    ) {
      payload[license] = orderState.vehicle.vehicleData.plate_id;
    }
    ...
    if (
      ...
      payload["preferred_date"] = orderDetails.dataAssignment.preferred_date.map((data) => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      clientDetails.place &&
      ((orderPrefillState && orderPrefillState.id) ||
        (dataAssignment && dataAssignment.preferred_date &&
  ...
  const deliveryOption = (orderState.deliveryOptions &&
    orderState.deliveryOptions.filter(
      (option: any) => option.id == orderDetails.dataAssignment.deliveryOption
    )[0]) || [{}];
                ...
                <div className="col-4">
                  ...
                  <Card className="shadow border-0">
                    <CardImg
                      top
                      width="100%"
                      src={orderState.vehicle.images}
                      alt="Vehicle Image"
                    />
                    <CardBody>
                      <h3 className="card-title">
                        {" "}
                        {orderState.vehicle.brand.name}{" "}
                        {orderState.vehicle.model.name}{" "}
                      </h3>
                      <CardSubtitle>
                        {" "}
                        {orderState.vehicle.fuel.name}{" "}
                        {orderState.vehicle.construction_year}{" "}
                      </CardSubtitle>
                      <CardSubtitle>
                        {" "}
                        <IntlMessages id={"vehicle.serviceExpire"} /> :{" "}
                        {orderState.vehicle.service_expire}
                      </CardSubtitle>
                      <CardSubtitle>
                        {" "}
                        <IntlMessages id={"vehicle.vehicleID"} /> :{" "}
                        {orderState.vehicle.vehicleData.vehicle_id}
                      </CardSubtitle>
                      {orderState.vehicle.vehicleData.plate_id ? (
                        <CardSubtitle>
                          {" "}
                            ...
                            orderDetails.dataAssignment.preferred_date.map(
                              ({ date, time }, i) => (
                                <Fragment key={i}>
                                  ...
                                  <div className="col-9">
                                    <h4>{time === "is_entire_day" ? "Hele dag beschikbaar" : time}</h4>
                                  </div>
                                </Fragment>
                              )
                            )}
