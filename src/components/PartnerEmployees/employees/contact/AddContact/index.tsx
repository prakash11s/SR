...
import {IAddContactProps,IAddContactState} from './Interface/IndexInterface'

class AddContact extends React.Component<IAddContactProps, IAddContactState> {
  constructor(props) {
    super(props);
          ...
          <div className="row no-gutters">
            ...
            <div className="col-lg-9 d-flex flex-column order-lg-1">
              <TextField
                required
                id="required"
                label={<IntlMessages id="partnerEmployee.name"/>}
                onChange={(event) => this.setState({name: event.target.value})}
                defaultValue={`${contact.first_name} ${contact.last_name}`}
                margin="none"/>
              <TextField
                id="required"
                label={<IntlMessages id="partnerEmployee.email"/>}
                onChange={(event) => this.setState({email: event.target.value})}
                value={email}
                margin="normal"/>
              <TextField
                id="required"
                label={<IntlMessages id="partnerEmployee.phone"/>}
              ...
              <TextField
                id="required"
                label={<IntlMessages id="partnerEmployee.role"/>}
