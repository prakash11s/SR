...
import AddContact from 'components/PartnerEmployees/employees/contact/AddContact';
import IntlMessages from 'util/IntlMessages';
import CustomScrollbars from 'util/CustomScrollbars';
import { getPartnerEmployees } from 'actions/Actions/PartnerEmployeeActions';
import { IPartnerEmployeesProps, IPartnerEmployeesState, IRootReducerPartnerEmployeesState } from './Interface/indexInterface';
...
const filterOptions = [
  ...
  {
    id: 3,
    name: 'Starred employees',
    icon: 'zmdi-star'
  }
];


class PartnerEmployees extends Component<IPartnerEmployeesProps, IPartnerEmployeesState> {
    ...
    return <div className="module-side">
      <div className="module-side-header">
        <div className="module-logo">
          ...
          <span><IntlMessages id="partner.employees"/></span>
        </div>
      </div>
        ...
        <CustomScrollbars className="module-side-scroll scrollbar"
          ...
          <div className="module-add-task">
            <Button className="jr-btn btn-block" variant="contained" color="primary" aria-label="add"
              ...
              <span><IntlMessages id="partner.addNewEmployee"/></span>
            </Button>
          </div>
  ...
  filterContact = (userName) => {
    ...
    if (userName === '') {
      ...
      const filterContact = this.state.allContact.filter((contact) =>
        contact.first_name.toLowerCase().indexOf(userName.toLowerCase()) > -1);
      if (filterOption === 'All employees') {
        this.setState({contactList: filterContact});
      } else if (filterOption === 'Frequently contacted') {
        this.setState({contactList: filterContact.filter((contact) => contact.frequently)});

      } else if (filterOption === 'Starred employees') {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      allContact: props.partnerEmployees ? props.partnerEmployees : [],
      contactList: props.partnerEmployees ? props.partnerEmployees : [],
  ...
  componentDidMount() {   
    this.props.getPartnerEmployees(25, 1);    
}

componentDidUpdate(prevProps) {  
  if(this.props.partnerEmployees !== prevProps.partnerEmployees){
    this.setState({...this.state, allContact: this.props.partnerEmployees, contactList: this.props.partnerEmployees})
  }
}
        ...
        <div className="app-module animated slideInUpTiny animation-duration-3">
          ...
          <div className="module-box">
            <div className="module-box-header">
              ...
              <AppModuleHeader placeholder="Search employee" notification={false} apps={false}
...
const mapStateToProps = ({settings, partnerEmployeeState, auth} : IRootReducerPartnerEmployeesState) => {
  const {width} = settings;
  const {partnerEmployees} = partnerEmployeeState;
  const {first_name, last_name, avatar} = auth.authUser
  return {width, partnerEmployees, first_name, last_name, avatar}
};

const mapDispatchToProps = (dispatch: any) => {    
  return {
    getPartnerEmployees: (limit:number, page:number) => dispatch(getPartnerEmployees(limit, page))  
  }
};
